...
import 'package:med/features/etudiant/data/Models/student_model.dart'; // for debugPrint
...
class StudentRemoteDataSource {
  ...
  /// Add a document link (Google Drive link) - OLD VERSION
  ...
  /// Add a StudentDocument object
  Future<void> addDocumentToStudent(String studentId, StudentDocument newDoc) async {
    debugPrint('📌 Adding document to student $studentId: ${newDoc.fileName}');
    try {
      final studentRef = firestore.collection('users').doc(studentId);
      await studentRef.update({
        'documents': FieldValue.arrayUnion([newDoc.toMap()]),
      });
    } catch (e) {
      debugPrint('❌ Error adding document to student: $e');
      rethrow;
    }
  }
